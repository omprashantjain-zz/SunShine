...
import com.woodbug.sunshine.data.WeatherContract;
...
public class ForecastAdapter extends RecyclerView.Adapter<ForecastAdapter.ForecastAdapterViewHolder> {
    ...
    final private com.woodbug.sunshine.ItemChoiceManager mICM;
    ...
    public ForecastAdapter(Context context, ForecastAdapterOnClickHandler dh, View emptyView, int choiceMode) {
        ...
        mICM = new com.woodbug.sunshine.ItemChoiceManager(this);
    ...
    public void onBindViewHolder(ForecastAdapterViewHolder forecastAdapterViewHolder, int position) {
        ...
        int weatherId = mCursor.getInt(com.woodbug.sunshine.ForecastFragment.COL_WEATHER_CONDITION_ID);
        int defaultImage;
        boolean useLongToday;

        switch (getItemViewType(position)) {
            case VIEW_TYPE_TODAY:
                defaultImage = com.woodbug.sunshine.Utility.getArtResourceForWeatherCondition(weatherId);
                useLongToday = true;
                break;
            default:
                defaultImage = com.woodbug.sunshine.Utility.getIconResourceForWeatherCondition(weatherId);
                useLongToday = false;
        }

        if ( com.woodbug.sunshine.Utility.usingLocalGraphics(mContext) ) {
            forecastAdapterViewHolder.mIconView.setImageResource(defaultImage);
        } else {
            Glide.with(mContext)
                    .load(com.woodbug.sunshine.Utility.getArtUrlForWeatherCondition(mContext, weatherId))
                    .error(defaultImage)
                    .crossFade()
        ...
        long dateInMillis = mCursor.getLong(com.woodbug.sunshine.ForecastFragment.COL_WEATHER_DATE);

        // Find TextView and set formatted date on it
        forecastAdapterViewHolder.mDateView.setText(com.woodbug.sunshine.Utility.getFriendlyDayString(mContext, dateInMillis, useLongToday));

        // Read weather forecast from cursor
        String description = com.woodbug.sunshine.Utility.getStringForWeatherCondition(mContext, weatherId);
        ...
        double high = mCursor.getDouble(com.woodbug.sunshine.ForecastFragment.COL_WEATHER_MAX_TEMP);
        String highString = com.woodbug.sunshine.Utility.formatTemperature(mContext, high);
        ...
        double low = mCursor.getDouble(com.woodbug.sunshine.ForecastFragment.COL_WEATHER_MIN_TEMP);
        String lowString = com.woodbug.sunshine.Utility.formatTemperature(mContext, low);
